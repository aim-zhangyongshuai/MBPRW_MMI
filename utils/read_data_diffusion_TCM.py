...
#     genes = list(pd.read_csv(path_par+'/data/TCM/output_file.csv')['Symbol'].str.strip().values)
#
...
def seed_gordon():
    import pandas as pd
    path_cur = os.getcwd()
    path_par = path_cur  # 或者按照你的需求指定疾病数据所在的路径
    df = pd.read_csv(path_par + '/data/TCM/output_file.csv')

    # 选择所需的列
    selected_columns = ['Symptom', 'EntrezID', 'Symbol']
...
def drugbank():
    ...
    #path_par = os.path.abspath(os.path.join(path_cur, os.pardir))
    path_par =  path_cur
    df = pd.read_csv(path_par + '/data/TCM/S4-1. HIT_herb_target_data_0412dropna.csv')
    data= zip(df['tcm_id'].values, df['entrez_id'].values)
    data_names = zip(df['tcm_id'].values, df['Chinese Pin Yin'].values)
    drug2target={}
