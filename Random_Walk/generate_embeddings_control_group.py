...
#         )
#
#         with open(f'results_大论文/exp_12.24/control_embeddings_100/tmp_walk_file{i}.pkl', 'wb') as fw:
...
#
#         outfile = os.path.join(output_dir, f'control_embedding_TCM_{i}.pkl')
...
import multiprocessing
from Random_Walk.HeterogeneousSG import HeterogeneousSG
from Random_Walk.utils import read_graph
from typing import List, Dict
from functools import partial
...
def generate_random_walks_control_parallel(
    ...
    walk_length: int,
    num_walks: int,
    start_nodes: List[str],          # ✅ 直接传入起点列表（drug+disease）
    seed: int = 43,
    workers: int = os.cpu_count()
) -> List[List[str]]:
    # 准备任务参数列表 (start_node, walk_length, seed)
...
def run_random_walk_control_experiments(
    ...
    # ✅ 起点 = 所有 drug+disease 节点（不抽样）
    start_nodes = [n for n in G.nodes() if node_types.get(n) in {'drug', 'disease'}]
    if len(start_nodes) == 0:
        raise ValueError("未找到 drug/disease 节点作为起点：请检查 nodetypef 与图节点ID是否一致。")

    os.makedirs(output_dir, exist_ok=True)
    tmp_dir = 'results_大论文/exp_12.24/control_embeddings_100'
    ...
    for i in range(num_trials):
        ...
        walks = generate_random_walks_control_parallel(
            ...
            walk_length=walk_length,
            num_walks=num_walks,
            start_nodes=start_nodes,   # ✅ 核心：起点只来自 drug + disease
            seed=seed,
            workers=workers
        )

        with open(os.path.join(tmp_dir, f'tmp_walk_file{i}.pkl'), 'wb') as fw:
            pickle.dump(walks, fw)

        print('Generating embeddings...')
        use_hetSG = True if nodetypef is not None else False
        embeddings = HeterogeneousSG(
            use_hetSG, walks, all_nodes_set, nodetypef=nodetypef,
            embedding_size=dimension, window_length=window_size, workers=workers
        )

        outfile = os.path.join(output_dir, f'control_embedding_drug_{i}.pkl')
        with open(outfile, 'wb') as fw:
            pickle.dump(embeddings, fw)
        print(f'Embeddings saved to {outfile}')
