...
def parse_args():
    ...
    parser.add_argument('--model_checkpoint', type=str, default='clf_TCM.pkl')
...
def split_dataset(pair_file, embedding_file, valid_ratio, test_ratio, seed):
    ...
    for line in lines:
        ...
        drug_emb = embedding_dict.get(drug)
        dis_emb = embedding_dict.get(dis)
        if drug_emb is None or dis_emb is None:
            continue  # Skip if any item is missing
    ...
    # Handling the scenario where the test set accounts for 100%
    if test_ratio >= 1.0:
    ...
    # Cross-check to ensure no data leakage
    if pair_data['train'] is not None and pair_data['test'] is not None:
        assert len(set(pair_data['train']) & set(pair_data['test'])) == 0, "There is overlap between the training set and the test set, resulting in data leakage！"
    if pair_data['train'] is not None and pair_data['valid'] is not None:
        assert len(set(pair_data['train']) & set(pair_data['valid'])) == 0, "There is overlap between the training set and the test set, resulting in data leakage！"
    if pair_data['valid'] is not None and pair_data['test'] is not None:
        assert len(set(pair_data['valid']) & set(pair_data['test'])) == 0, "There is overlap between the training set and the test set, resulting in data leakage！"

    return x, y, pair_data

def return_scores(target_list, pred_list):
    """
    Calculate and return multiple evaluation metrics of the model:
    1. AUROC
    2. Global AUPR
    3. Average precision of the top 1% predicted items (Precision@1%)
    4. Recall rate of the top 1% predicted items (Recall@1%)
    """
    # Input inspection
...
def predict_asso(embedding_file, pair_file,clinic_predict_file_path, model_checkpoint, seed, valid_ratio, test_ratio, train=True):
    ...
    print(f"\nTEST集 | AUC: {scores['AUROC'] * 100:.2f}% | "
          f"Precision@Top1%: {scores['Precision (Top 1%)']:.4f} | Recall@Top1%: {scores['Recall (Top 1%)']:.4f}")

    # Add the following code at the end of the prediction code block (replace the original txt saving section):
    if test_ratio==1.0:
        with open(clinic_predict_file_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["drug", "disease", "label","score"])  # CSV头部
